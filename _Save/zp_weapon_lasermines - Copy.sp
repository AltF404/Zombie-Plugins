...
#define WEAPON_BEAM_HUMAN_COLOR    {30, 144, 255, 255}
#define WEAPON_BEAM_HUMAN_COLOR_F  "30 144 255"
#define WEAPON_GLOW_HUMAN_COLOR    0, 255, 0, 255
#define WEAPON_BEAM_ZOMBIE_COLOR   {30, 144, 255, 255}
#define WEAPON_BEAM_ZOMBIE_COLOR_F "30 144 255"
#define WEAPON_GLOW_ZOMBIE_COLOR   30, 144, 255, 255
...
public void OnPluginStart()
{
	...
	hCvarMineDamage  = CreateConVar("zp_weapon_lasermine_damage", "35000.0", "Damage amount", 0, true, 0.0);
	RegAdminCmd("sm_explodemines", Command_ExplodeMines, ADMFLAG_SLAY);
...
public Action Command_ExplodeMines(int client, int args)
{
	ExplodeAllLaserMines();
}
public void ExplodeAllLaserMines()
{
	int index = -1;
	while ((index = FindEntityByClassname(index, "prop_physics_multiplayer")) != -1)
	{
		if (IsEntityLasermine(index))
		{
			// Destroy damage hook
			SDKUnhook(index, SDKHook_OnTakeDamage, MineDamageHook);
...
/*public void ZP_OnGameModeStart(int mode)
{
	...
	ZP_GetGameModeName(mode, gamemodeName, sizeof(gamemodeName));
	//
	if (StrContains("sirena mode|bomber mode|survivor mode|sniper mode|armageddon mode|pvs mode|berserker mode|wesker mode|spyclass mode|remix mode", gamemodeName, false) != -1)
	{
		ExplodeAllLaserMines();
	}
}*/
