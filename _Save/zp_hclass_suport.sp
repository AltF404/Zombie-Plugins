...
{
	hCvarSkillReward = CreateConVar("zp_hclass_suport_reward", "1", "Reward for healing each zombie", 0, true, 0.0);
	hCvarSkillRadius = CreateConVar("zp_hclass_suport_radius", "250.0", "Healing radius", 0, true, 0.0);
	hCvarSkillEffect = CreateConVar("zp_hclass_suport_effect", "tornado", "Particle effect for the skill (''-default)");
	hCvarSkillHeal   = CreateConVar("zp_hclass_suport_heal", "heal_ss", "Particle effect for the heal (''-off)");
	
	AutoExecConfig(true, "zp_hclass_suport", "sourcemod/zombieplague");
}
...
{
	gZombie = ZP_GetClassNameID("suport");
