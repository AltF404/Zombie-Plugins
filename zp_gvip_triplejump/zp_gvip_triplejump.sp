...
{
	name		= "Double Jump VIP",
	author		= "AltF4",
	description	= "Allows vips to double-jumping.",
...
{
	CreateConVar("sm_mjump_version", PLUGIN_VERSION, "Multijump Version", 0 | FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DONTRECORD)
	g_cvJumpKnife = CreateConVar("sm_mjump_knife", "0",	"disable(0) / enable(1) multi-jumping only on Knife Level for AR (GunGame)",0, true, 0.0, true, 1.0)
	g_cvJumpBoost = CreateConVar("sm_mjump_boost", "300.0", "The amount of vertical boost to apply to double jumps",0)
	g_cvJumpMax = CreateConVar("sm_mjump_max", "1", "Max rejumps count; 0 - disable plugin",0, true, 0.0)
	g_cvJumpAnnounce = CreateConVar("sm_mjump_announce", "0", "Should this plugin announce itself?",0, true, 0.0, true, 1.0)
